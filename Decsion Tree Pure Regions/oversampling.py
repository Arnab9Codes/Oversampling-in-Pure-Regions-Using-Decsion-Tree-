...
def OverSampling(location_of_data,test_data):
    ...
    for FILE in files:
        if (FILE!=test) and (not(FILE.endswith('tst.dat')) and ((FILE[len(FILE)-8])!=(test_data[len(test_data)-8])) ):
            
            #print(FILE)
            df=pd.read_csv('../Data_tsai/NonSMOTE/glass0/glass0-5-fold/'+FILE)
            l.append(df)
    ...
    pure_samples_X_train,pure_samples_with_class,rules_with_pure_class=SampleExtraction.pure_sample_extraction(best_tree,X,Y,cls[0][1])
